...
class CNN_And_LSTM(nn.Module):
	def __init__(self, **kwargs):
		super(CNN_And_LSTM, self).__init__()
	...
	def forward(self, x):
		...
		# LSTM
		x_lstm = self.embedding(x)
		x_lstm = x_lstm.transpose(0,1)	
		lstm_out, self.hidden = self.LSTM(x_lstm, self.hidden)
		lstm_out = lstm_out[-1].squeeze(0)
		lstm_out = lstm_out.unsqueeze(0)	

		# CNN and LSTM concat
		cnn_lstm_out = torch.cat((cnn_out, lstm_out),2)
		cnn_lstm_out = cnn_lstm_out.squeeze(0)
